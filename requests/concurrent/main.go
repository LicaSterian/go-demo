...
func main() {
	...
	for i := 0; i < numConcurrentRequests; i++ {
		wg.Add(1)
		userEmail := fmt.Sprintf("user_%d@hcltech.com", i+100)
		go createNewUser(&wg, userEmail)
	}
	wg.Wait()

	took := time.Since(startTime)
	log.Printf("%d concurrent requests took: %v", numConcurrentRequests, took)
}

func createNewUser(wg *sync.WaitGroup, userEmail string) {
	defer wg.Done()
